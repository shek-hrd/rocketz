/**
 * Rocket Text Game v3 - Enhanced Features
 * Speed scaling, health opacity, text wrapping, and improved controls
 */

class RocketTextGameV3 {
    ...
    static init(textareaId) {
        ...
        if (RocketTextGameV3.instances.has(textareaId)) {
            return RocketTextGameV3.instances.get(textareaId);
        }
        
        const instance = new RocketTextGameV3(textarea);
        RocketTextGameV3.instances.set(textareaId, instance);
        return instance;
    }
    ...
    createGameElements() {
        ...
        this.container.className = 'rocket-game-container-v3';
        ...
        this.rocket.className = 'rocket-game-rocket-v3';
        ...
        this.healthBar.className = 'rocket-game-health-v3';
        ...
        this.pointsDisplay.className = 'rocket-game-points-v3';
        ...
        style.textContent = `
            @keyframes rocket-blink-v3 {
                0%, 50% { opacity: 1; }
                51%, 100% { opacity: 0; }
            }
            
            .rocket-game-rocket-v3.blinking {
                animation: rocket-blink-v3 0.3s infinite;
            }
            
            @keyframes damage-fade-v3 {
                0% { opacity: 1; transform: translateY(0); }
                100% { opacity: 0; transform: translateY(-20px); }
            }
            
            .rocket-damage-text-v3 {
                position: absolute;
                color: #ff4444;
                font-weight: bold;
                font-size: 12px;
                pointer-events: none;
                z-index: 25;
                animation: damage-fade-v3 1s forwards;
            }
            
            .rocket-game-container-v3.rocket-mode textarea {
                pointer-events: none;
            }
        `;
    ...
    deactivateRocketMode() {
        ...
        this.rocket.firstChild.style.opacity = '1';
    }
    ...
    updateRocket() {
        ...
        if (this.keys.ArrowDown) {
            this.rocketSpeed = Math.max(this.rocketSpeed - 0.2, 0.5); // Minimum speed to prevent stopping
        }
        ...
        // Handle text wrapping - both horizontal and vertical
    ...
    updatePoints() {
        if (this.rocketSpeed > 0.5) {
            ...
            // Update max speed based on points
            this.maxSpeed = this.baseMaxSpeed + Math.floor(this.points / 10) * 0.5;
            this.maxSpeed = Math.min(this.maxSpeed, 10); // Cap at max 10
        }
    }
    ...
    updateRocketOpacity() {
        ...
        this.rocket.firstChild.style.opacity = opacity.toString();
        
        // Flashing effect when health is very low
        if (this.health <= 10) {
            if (!this.opacityInterval) {
                this.opacityInterval = setInterval(() => {
                    const currentOpacity = parseFloat(this.rocket.firstChild.style.opacity);
                    this.rocket.firstChild.style.opacity = currentOpacity === 0 ? '0.5' : '0';
                }, 200);
            }
    ...
    updatePointsDisplay() {
        ...
        // Dynamic positioning based on acceleration/deceleration
        let distance = 30 + (this.rocketSpeed * 10);
        
        // Adjust distance based on acceleration state
        if (this.acceleration > 0) {
            distance += 15; // Further when accelerating
        } else if (this.acceleration < 0) {
            distance -= 10; // Closer when decelerating
        }
        ...
        // Add slight scale effect based on speed
    ...
    destroy() {
        ...
        RocketTextGameV3.instances.delete(this.textarea.id);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    elements.forEach(el => {
        RocketTextGameV3.init(el.id);
    });
});

// Export for module systems
if (typeof module !== 'undefined' && module.exports) {
    module.exports = RocketTextGameV3;
} else if (typeof window !== 'undefined') {
    window.RocketTextGameV3 = RocketTextGameV3;
}